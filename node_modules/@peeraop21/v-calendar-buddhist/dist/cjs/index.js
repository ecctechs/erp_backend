...
function createCalendar(props, { emit, slots }) {
  ...
  const firstPage = vue.computed(() => head_1(state2.pages) ?? null);
  const lastPage = vue.computed(() => last_1(state2.pages) ?? null);
...
function createCalendarNav(props, { emit }) {
  ...
  const title2 = vue.computed(() => {
    ...
    return monthMode.value ? yearIndexTran : `${firstYearTran} - ${lastYearTran}`;
  });
